#!/usr/bin/env bash
# Reproduces the Lean model (TransactShim + CheckPublicAmountShim) from the zkcash
# source and builds it. Run from formal-verification/, inside `nix develop path:.`.
# Requires setup-vendor.sh + cargo-config.toml.template only if charon hits the cdylib
# link error (macOS; see SETUP.md Part 2 -- untested on Linux).
#
# Modes:
#   ./extract.sh                regenerate the model (charon -> .llbc -> aeneas -> lean),
#                               materialize the *External imports, then `lake build` it.
#   ./extract.sh --diagnose-fr  fork-A experiment: does --monomorphize let Charon
...
# ---------------------------------------------------------------------------
# Fork-A diagnostic (opt-in): can Charon translate the REAL Fr via monomorphization,
...
# useful as a DIAGNOSIS of *why* Fr crashes, not as the production model -- see the
# go/no-go discussion / MODEL_REPORT.md.
# ---------------------------------------------------------------------------
if [ "$MODE" = "diagnose-fr" ]; then
  echo "=== Fork A: does --monomorphize let Charon translate the real Fr? ==="
...
eval "charon rustc \
  ...
  --opaque \"zkcash::fv_verify_proof_entry\" \
...
BAK=$(mktemp -d)
for m in TransactShim CheckPublicAmountShim; do
...
rm -rf lean/CheckPublicAmountShim lean/TransactShim
...
for m in TransactShim CheckPublicAmountShim; do
  for f in TypesExternal FunsExternal; do
    if [ -f "$BAK/${m}__$f.lean" ]; then
      ...
      if [ -f "$BAK/${m}__${f}_Template.lean" ] && \
...
# already-patched sites.) See MODEL_REPORT.md and the lean-model-stale-vs-pinned-aeneas note.
for m in TransactShim CheckPublicAmountShim; do
...
echo "Done. lean/{TransactShim,CheckPublicAmountShim}/ regenerated AND built."
